...
class RegisterHSFCPlayer6 extends React.Component {
    state = {
        loading: true,
        saving: false,
        name: '',
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            databaseRef.child('player6Exist').once('value')
            .then(player6Exist => {
                if (player6Exist.val()) {
                    databaseRef.child('player6').once('value')
                    .then(player6 => this.setState({
                        name: player6.val().name,
                        birthPlace: player6.val().birthPlace,
                        birthDate: player6.val().birthDate,
                        backNumber: player6.val().backNumber,
                        contact: player6.val().contact,
                        photo: player6.val().photo,
                        studentCard: player6.val().studentCard,
                        reportCard: player6.val().reportCard,
                        loading: false
                    }))
                } else {
    ...
    onSubmit(e) {
        ...
        databaseRef.child('player6').update({
        ...
        .then(() => router.push('/register/hsfc/player7'))
    ...
    onPhotoUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player6')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player6').child('photo').child(file.name)
    ...
    onStudentCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player6')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player6').child('studentCard').child(file.name)
    ...
    onReportCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player6')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player6').child('reportCard').child(file.name)
    ...
    render() {
        ...
        return (
            <div>
                <Container className="pt-md-5 pt-sm-6">
                    <RegisterHSFCTab active={3}/>
                    <RegisterHSFCPlayerTab active={6}/>
                    ...
                    <div className="p-3 bg-light rounded border mb-5">
                        <Row>
                            <Col md="6" sm="6" xs="6">
                                <Button type="reset" color="outline-secondary" size="sm" onClick={() => router.push('/register/hsfc/player5')}>Kembali</Button>
                            </Col>
                            <Col md="6" sm="6" xs="6">
                                ...
                                <Button type="reset" size="sm" color="outline-warning" className="float-right mr-2" onClick={() => router.push('/register/hsfc/player7')}>Lewati</Button>
                            </Col>
                        </Row>
                    </div>
                    </Form> }
...
export default withRouter(RegisterHSFCPlayer6)