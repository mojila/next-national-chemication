...
class RegisterHSFCPlayer8 extends React.Component {
    state = {
        loading: true,
        saving: false,
        name: '',
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            databaseRef.child('player8Exist').once('value')
            .then(player8Exist => {
                if (player8Exist.val()) {
                    databaseRef.child('player8').once('value')
                    .then(player8 => this.setState({
                        name: player8.val().name,
                        birthPlace: player8.val().birthPlace,
                        birthDate: player8.val().birthDate,
                        backNumber: player8.val().backNumber,
                        contact: player8.val().contact,
                        photo: player8.val().photo,
                        studentCard: player8.val().studentCard,
                        reportCard: player8.val().reportCard,
                        loading: false
                    }))
                } else {
    ...
    onSubmit(e) {
        ...
        databaseRef.child('player8').update({
        ...
        .then(() => router.push('/register/hsfc/player9'))
    ...
    onPhotoUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player8')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player8').child('photo').child(file.name)
    ...
    onStudentCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player8')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player8').child('studentCard').child(file.name)
    ...
    onReportCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player8')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player8').child('reportCard').child(file.name)
    ...
    render() {
        ...
        return (
            <div>
                <Container className="pt-md-5 pt-sm-8">
                    <RegisterHSFCTab active={3}/>
                    <RegisterHSFCPlayerTab active={8}/>
                    ...
                    <div className="p-3 bg-light rounded border mb-5">
                        <Row>
                            ...
                            <Col md="6" sm="6" xs="6">
                                ...
                                <Button type="reset" size="sm" color="outline-warning" className="float-right mr-2" onClick={() => router.push('/register/hsfc/player9')}>Lewati</Button>
                            </Col>
                        </Row>
                    </div>
                    </Form> }
...
export default withRouter(RegisterHSFCPlayer8)