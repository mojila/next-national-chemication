...
class RegisterECMember2 extends React.Component {
    state = {
        loading: true,
        saving: false,
        photoSaving: false,
    ...
    photoUpload(e) {
        ...
        let databaseRef = database.ref('pesertaEC/'+uid).child('member2')
        let storageRef = storage.ref('pesertaEC/'+uid).child('member2').child('photo').child(file.name)
    ...
    studentIDUpload(e) {
        ...
        let uid = localStorage.getItem('ec-uid')
        let databaseRef = database.ref('pesertaEC/' + uid).child('member2')
        let storageRef = storage.ref('pesertaEC/' + uid).child('member2').child('studentID').child(file.name)
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            database.ref('pesertaEC/'+uid).child('member2Exist').once('value')
                .then((member2Exist) => {
                    if (member2Exist.val()) {
                        database.ref('pesertaEC/'+uid).child('member2').once('value')
                            .then((member2) => this.setState({ 
                                loading: false,
                                name: member2.val().name,
                                nim: member2.val().nim,
                                majors: member2.val().majors,
                                semester: member2.val().semester,
                                member2Exist: member2Exist.val(),
                                email: member2.val().email,
                                photo: member2.val().photo,
                                studentID: member2.val().studentID,
                                contact: member2.val().contact
                            }))
                    } else {
    ...
    onSubmit(e) {
        ...
        database.ref('pesertaEC/'+uid).child('member2').update({
            name, nim, majors, semester, photo, studentID, contact, email
        })
        .then(() => database.ref('pesertaEC/'+uid).update({ member2Exist: true }))
        .then(() => router.push('/register/ec/confirmation'))
                            ...
                            <Row>
                                <Col md="4">
                                    <FormGroup>
                                        ...
                                        <Input type="number" min={1} size="sm" value={ nim } onChange={ e => this.setState({ nim: e.target.value }) } required/>
                                    </FormGroup>
                                </Col>
                                <Col md="4">
                                    <FormGroup>
...
export default withRouter(RegisterECMember2)