...
class RegisterHSFCPlayer10 extends React.Component {
    state = {
        loading: true,
        saving: false,
        name: '',
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            databaseRef.child('player10Exist').once('value')
            .then(player10Exist => {
                if (player10Exist.val()) {
                    databaseRef.child('player10').once('value')
                    .then(player10 => this.setState({
                        name: player10.val().name,
                        birthPlace: player10.val().birthPlace,
                        birthDate: player10.val().birthDate,
                        backNumber: player10.val().backNumber,
                        contact: player10.val().contact,
                        photo: player10.val().photo,
                        studentCard: player10.val().studentCard,
                        reportCard: player10.val().reportCard,
                        loading: false
                    }))
                } else {
    ...
    onSubmit(e) {
        ...
        databaseRef.child('player10').update({
        ...
        .then(() => router.push('/register/hsfc/confirmation'))
    ...
    onPhotoUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player10')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player10').child('photo').child(file.name)
    ...
    onStudentCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player10')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player10').child('studentCard').child(file.name)
    ...
    onReportCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player10')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player10').child('reportCard').child(file.name)
    ...
    render() {
        ...
        return (
            <div>
                <Container className="pt-md-5 pt-sm-10">
                    <RegisterHSFCTab active={3}/>
                    <RegisterHSFCPlayerTab active={10}/>
                    ...
                    <div className="p-3 bg-light rounded border mb-5">
                        <Row>
                            <Col md="6" sm="6" xs="6">
                                <Button type="reset" color="outline-secondary" size="sm" onClick={() => router.push('/register/hsfc/player9')}>Kembali</Button>
                            </Col>
                            <Col md="6" sm="6" xs="6">
                                ...
                                <Button type="reset" size="sm" color="outline-warning" className="float-right mr-2" onClick={() => router.push('/register/hsfc/confirmation')}>Lewati</Button>
                            </Col>
                        </Row>
                    </div>
                    </Form> }
...
export default withRouter(RegisterHSFCPlayer10)