...
class RegisterHSFCPlayer9 extends React.Component {
    state = {
        loading: true,
        saving: false,
        name: '',
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            databaseRef.child('player9Exist').once('value')
            .then(player9Exist => {
                if (player9Exist.val()) {
                    databaseRef.child('player9').once('value')
                    .then(player9 => this.setState({
                        name: player9.val().name,
                        birthPlace: player9.val().birthPlace,
                        birthDate: player9.val().birthDate,
                        backNumber: player9.val().backNumber,
                        contact: player9.val().contact,
                        photo: player9.val().photo,
                        studentCard: player9.val().studentCard,
                        reportCard: player9.val().reportCard,
                        loading: false
                    }))
                } else {
    ...
    onSubmit(e) {
        ...
        databaseRef.child('player9').update({
        ...
        .then(() => router.push('/register/hsfc/player10'))
    ...
    onPhotoUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player9')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player9').child('photo').child(file.name)
    ...
    onStudentCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player9')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player9').child('studentCard').child(file.name)
    ...
    onReportCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player9')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player9').child('reportCard').child(file.name)
    ...
    render() {
        ...
        return (
            <div>
                <Container className="pt-md-5 pt-sm-9">
                    <RegisterHSFCTab active={3}/>
                    <RegisterHSFCPlayerTab active={9}/>
                    ...
                    <div className="p-3 bg-light rounded border mb-5">
                        <Row>
                            <Col md="6" sm="6" xs="6">
                                <Button type="reset" color="outline-secondary" size="sm" onClick={() => router.push('/register/hsfc/player8')}>Kembali</Button>
                            </Col>
                            <Col md="6" sm="6" xs="6">
                                ...
                                <Button type="reset" size="sm" color="outline-warning" className="float-right mr-2" onClick={() => router.push('/register/hsfc/player10')}>Lewati</Button>
                            </Col>
                        </Row>
                    </div>
                    </Form> }
...
export default withRouter(RegisterHSFCPlayer9)