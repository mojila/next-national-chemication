...
class RegisterHSFCPlayer1 extends React.Component {
    state = {
        loading: true,
        saving: false,
        name: '',
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            databaseRef.child('player1Exist').once('value')
            .then(player1Exist => {
                if (player1Exist.val()) {
                    databaseRef.child('player1').once('value')
                    .then(player1 => this.setState({
                        name: player1.val().name,
                        birthPlace: player1.val().birthPlace,
                        birthDate: player1.val().birthDate,
                        backNumber: player1.val().backNumber,
                        contact: player1.val().contact,
                        photo: player1.val().photo,
                        studentCard: player1.val().studentCard,
                        reportCard: player1.val().reportCard,
                        loading: false
                    }))
                } else {
    ...
    onSubmit(e) {
        ...
        databaseRef.child('player1').update({
        ...
        .then(() => router.push('/register/hsfc/player2'))
    ...
    onPhotoUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player1')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player1').child('photo').child(file.name)
    ...
    onStudentCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player1')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player1').child('studentCard').child(file.name)
    ...
    onReportCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player1')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player1').child('reportCard').child(file.name)
        ...
        return (
            <div>
                <Container className="pt-md-5 pt-sm-1">
                    ...
                    <RegisterHSFCPlayerTab active={1}/>
                    ...
                    <div className="p-3 bg-light rounded border mb-5">
                        <Row>
                            <Col md="6" sm="6" xs="6">
                                <Button type="reset" color="outline-secondary" size="sm" onClick={() => router.push('/register/hsfc/official2')}>Kembali</Button>
                            </Col>
                            <Col md="6" sm="6" xs="6">
...
export default withRouter(RegisterHSFCPlayer1)