...
class RegisterHSFCPlayer4 extends React.Component {
    state = {
        loading: true,
        saving: false,
        name: '',
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            databaseRef.child('player4Exist').once('value')
            .then(player4Exist => {
                if (player4Exist.val()) {
                    databaseRef.child('player4').once('value')
                    .then(player4 => this.setState({
                        name: player4.val().name,
                        birthPlace: player4.val().birthPlace,
                        birthDate: player4.val().birthDate,
                        backNumber: player4.val().backNumber,
                        contact: player4.val().contact,
                        photo: player4.val().photo,
                        studentCard: player4.val().studentCard,
                        reportCard: player4.val().reportCard,
                        loading: false
                    }))
                } else {
    ...
    onSubmit(e) {
        ...
        databaseRef.child('player4').update({
        ...
        .then(() => router.push('/register/hsfc/player5'))
    ...
    onPhotoUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player4')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player4').child('photo').child(file.name)
    ...
    onStudentCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player4')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player4').child('studentCard').child(file.name)
    ...
    onReportCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player4')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player4').child('reportCard').child(file.name)
    ...
    render() {
        ...
        return (
            <div>
                <Container className="pt-md-5 pt-sm-4">
                    <RegisterHSFCTab active={3}/>
                    <RegisterHSFCPlayerTab active={4}/>
                    ...
                    <div className="p-3 bg-light rounded border mb-5">
                        <Row>
                            <Col md="6" sm="6" xs="6">
                                <Button type="reset" color="outline-secondary" size="sm" onClick={() => router.push('/register/hsfc/player3')}>Kembali</Button>
                            </Col>
                            <Col md="6" sm="6" xs="6">
...
export default withRouter(RegisterHSFCPlayer4)