...
class Member2CEO extends React.Component {
    state = {
        member2Exist: '',
        error: "",
        name: "",
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            let member2Fetch = database.ref('pesertaCEO/' + uid).child('member2')

            member2ExistFetch.once('value').then((member2Exist) => {
                if (member2Exist.val()) {
                    member2Fetch.once('value').then((member2) => {
                        let res = member2.val()
                    ...
                    this.setState({ loading: false, member2Exist: false })
                }
            })
        }
    }
    ...
    photoUpload(e) {
        ...
        let uid = localStorage.getItem('ceo-uid')
        let databaseRef = database.ref('pesertaCEO/' + uid).child('member2')
        let storageRef = storage.ref('pesertaCEO/' + uid).child('member2').child('photo').child(file.name)
    ...
    studentIDUpload(e) {
        ...
        let uid = localStorage.getItem('ceo-uid')
        let databaseRef = database.ref('pesertaCEO/' + uid).child('member2')
        let storageRef = storage.ref('pesertaCEO/' + uid).child('member2').child('studentID').child(file.name)
    ...
    onSubmit(e) {
        ...
        databaseRef.update({
            ...
            database.ref('pesertaCEO/' + uid).update({
                member2Exist: true
            }).then(() => this.setState({ saving: false, saved: true, member2Exist: true }))
        })
    ...
    render() {
        let { 
            ...
            name, gender, member2Exist, nisn, classLevel, 
                ...
                <Container className="pt-md-5">
                    ...
                    { !loading && <Form onSubmit={ this.onSubmit.bind(this) }>
                        <Row className="mb-2 p-3 bg-white shadow rounded">
                            <Col>
                                <p className="m-0 p-0 text-center">Anggota 2</p>
                            </Col>
                        </Row>
                        <Row className="mb-2 p-3 bg-white shadow rounded">
                            <Col md="3">
                        ...
                        <Row className="mb-2 p-3 bg-white shadow rounded">
                            <Col md="4">
                                <Button type="reset" block onClick={ () => router.push('/dashboard/ceo/member/member1') }>Kembali</Button>
                            </Col>
                            <Col md="4">
...
export default withRouter(Member2CEO)