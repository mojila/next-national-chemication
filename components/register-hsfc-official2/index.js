...
class RegisterHSFCOfficial2 extends React.Component {
    state = {
        loading: true,
        saving: false,
        photoSaving: false,
        official2Exist: false,
        photo: '',
        name: '',
        contact: ''
    }
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            databaseRef.child('official2Exist').once('value')
            .then((official2Exist) => {
                if (official2Exist.val()) {
                    databaseRef.child('official2').once('value')
                    .then((official2) => this.setState({
                        official2Exist: official2Exist.val(), 
                        name: official2.val().name,
                        contact: official2.val().contact,
                        photo: official2.val().photo,
                        loading: false
                    }))
                } else {
    ...
    onSubmit(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('official2')
        ...
        .then(() => router.push('/register/hsfc/player1'))
    ...
    onUploadPhoto(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('official2').child('photo').child(file.name)
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('official2')
        ...
        return (
            <div>
                <Container className="pt-md-5 pt-sm-1">
                    ...
                    <RegisterHSFCOfficialTab active={2}/>
                    ...
                    <div className="p-3 bg-light border rounded mb-5">
                        <Row>
                            <Col md="6" sm="6" xs="6">
                                <Button type="reset" color="outline-secondary" size="sm" onClick={() => router.push('/register/hsfc/official1')}>Kembali</Button>
                            </Col>
                            <Col md="6" sm="6" xs="6">
...
export default withRouter(RegisterHSFCOfficial2)