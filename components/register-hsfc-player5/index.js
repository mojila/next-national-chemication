...
class RegisterHSFCPlayer5 extends React.Component {
    state = {
        loading: true,
        saving: false,
        name: '',
    ...
    componentDidMount() {
        ...
        if (this.mounted) {
            ...
            databaseRef.child('player5Exist').once('value')
            .then(player5Exist => {
                if (player5Exist.val()) {
                    databaseRef.child('player5').once('value')
                    .then(player5 => this.setState({
                        name: player5.val().name,
                        birthPlace: player5.val().birthPlace,
                        birthDate: player5.val().birthDate,
                        backNumber: player5.val().backNumber,
                        contact: player5.val().contact,
                        photo: player5.val().photo,
                        studentCard: player5.val().studentCard,
                        reportCard: player5.val().reportCard,
                        loading: false
                    }))
                } else {
    ...
    onSubmit(e) {
        ...
        databaseRef.child('player5').update({
        ...
        .then(() => router.push('/register/hsfc/player6'))
    ...
    onPhotoUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player5')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player5').child('photo').child(file.name)
    ...
    onStudentCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player5')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player5').child('studentCard').child(file.name)
    ...
    onReportCardUpload(e) {
        ...
        let uid = localStorage.getItem('hsfc-uid')
        let databaseRef = database.ref('pesertaHSFC/'+uid).child('player5')
        let storageRef = storage.ref('pesertaHSFC/'+uid).child('player5').child('reportCard').child(file.name)
    ...
    render() {
        ...
        return (
            <div>
                <Container className="pt-md-5 pt-sm-5">
                    <RegisterHSFCTab active={3}/>
                    <RegisterHSFCPlayerTab active={5}/>
                    ...
                    <div className="p-3 bg-light rounded border mb-5">
                        <Row>
                            <Col md="6" sm="6" xs="6">
                                <Button type="reset" color="outline-secondary" size="sm" onClick={() => router.push('/register/hsfc/player4')}>Kembali</Button>
                            </Col>
                            <Col md="6" sm="6" xs="6">
...
export default withRouter(RegisterHSFCPlayer5)